...
int main(void)
{
    ...
    message("This program allows you to change the floor of the elevator control system,\n"); // Display an introductory message
    ...
    while (1) // Start an infinite loop
    {
        ...
        // Turn on LED
        else if (strcmp(command, "Floor 1") == 0) // Check if the user's command is "Turn on"
        {
            led_on();                      // Turn on the LED
            newStatus("Floor 1");          // Update the status
            message("\r\n >>  ");          // Display a prompt for the next command
        }

        // Turn off LED
        else if (strcmp(command, "Floor 2") == 0) // Check if the user's command is "Turn off"
        {
            led_off();                     // Turn off the LED
            newStatus("Floor 2");         // Update the status
            message("\r\n >>  ");          // Display a prompt for the next command
        }
        ...
        else if (strcmp(command, "Status") == 0) // Check if the user's command is "Status"
        {
            if (GPIOA->IDR & GPIO_IDR_IDR5)   // Check the status of the LED
            {
                ...
                message("\r\n Status: Elevator is on the 1st floor.\n"); // Display the LED status
                message("\r\n >>  ");         // Display a prompt for the next command
            }
            else
            {
                message("\r\n Status: Elevator is on the 2nd floor.\n"); // Display the LED status
                message("\r\n >>  ");                  // Display a prompt for the next command
            }
        }
        ...
        else if (strcmp(command, "Help") == 0) // Check if the user's command is "Help"
        {
            ...
            message("\r\n Enter 'Floor 1' to go to the first floor.\n");
            message("\r\n Enter 'Floor 2' to go to the second floor.\n");
            message("\r\n Enter 'Status' to find out the position of the elevator.\n");
